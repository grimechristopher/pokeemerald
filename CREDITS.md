...
- [rh-hideout/pokeemerald-expansion, `upcoming-decap` branch](https://github.com/rh-hideout/pokeemerald-expansion/tree/upcoming-decap) by Bassoonian, Martin Griffin, and grintoul - source of the `TEXT_CAPITALIZE`/`[bracket]` build-time capitalization mechanism this fork's decapitalization is built on
- [prof-harpe/pokeemerald-expansion, "About Decapitalized"](https://github.com/prof-harpe/pokeemerald-expansion) by Prof. Harpe - ~99% complete decapitalization of pokeemerald-expansion's text; this fork extracted its editorial decisions (which spans to recase) and replayed them as `[bracket]` tags against our own tree
...
- [volromhacking/pokeemerald-expansion, `mining_minigame` branch](https://github.com/volromhacking/pokeemerald-expansion/tree/mining_minigame) - source this fork's DPPt-style Underground Mining minigame was ported from, by Drin Emini (vol8), with review input from hedara90, khbsd, and AlexOn1ine
- [sphericalice/pokeemerald, `sliding_puzzles` branch](https://github.com/sphericalice/pokeemerald/tree/sliding_puzzles) - source this fork's Ruins of Alph-style sliding tile puzzles were ported from, by sphericalice, based on Jaizu's original pokefirered implementation ([M_X-RuinsofAlph](https://github.com/shinyquagsire23/M_X-RuinsofAlph) by Mastermind_X, via shinyquagsire23)
- [pret/pokeplatinum](https://github.com/pret/pokeplatinum) and [pret/pokeheartgold](https://github.com/pret/pokeheartgold) - consulted as design reference for this fork's Headbutt tree encounters (their `honey_tree.c`/`unk_02054E00.c`); not a direct port, since the DS games' 3D object/overlay system doesn't translate to GBA - this fork's implementation is built from scratch against pokeemerald's own metatile-behavior/field-move conventions
