#include "game_corner_flappybird.h"
...
void FlappyBird_Main(void)
{
    FlappyBirdMainCallback();
}
...
static void CreateScore(void)
{
	struct SpriteSheet s;
        ...
        LZ77UnCompWram(sSpriteSheet_Score.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateScore2(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Score2.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateFlap(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Flap.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateOne(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_One.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateTwo(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Two.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateThree(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Three.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateHiScore(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_HiScore.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
{
	struct SpriteSheet s;
        LZ77UnCompWram(sSpriteSheet_Butterfree_Hitbox.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
{
	struct SpriteSheet s;
        LZ77UnCompWram(sSpriteSheet_Border.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
{
	struct SpriteSheet s;
        LZ77UnCompWram(sSpriteSheet_Border_2.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
{
	struct SpriteSheet s;
        LZ77UnCompWram(sSpriteSheet_Butterfree.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateDamage(void)
{
	struct SpriteSheet s;
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Damage.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
{
	struct SpriteSheet s;
        LZ77UnCompWram(sSpriteSheet_Trail.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
	...
	switch (sFlappy->state)
	{
		...
		case FLAPPY_INPUT:
			...
			if ((sFlappy->bg2ScrollX == 225) || ((sFlappy->bg2ScrollX == 98) && (sFlappy->Obstacle1Id != 0))) // Point
			{
				...
				// Award coins at game over, not here
		...
		case FLAPPY_GAMEOVER:
			...
			if (sFlappy->timerDelay == 0)
			{
				...
				u16 coinsWon = sFlappy->Points * 2;
				GameCorner_AddCoins(coinsWon);

				// Check high score
				GameCorner_IncrementPlayCount(MINIGAME_FLAPPYBIRD);
				if (GameCorner_IsNewHighScore(MINIGAME_FLAPPYBIRD, sFlappy->Points))
				{
					PlayFanfare(MUS_LEVEL_UP);
					GameCorner_UpdateHighScore(MINIGAME_FLAPPYBIRD, sFlappy->Points);
...
static void InitFlappyBirdScreen(void)
{
	...
	SetScoreDigits(GameCorner_GetHighScore(MINIGAME_FLAPPYBIRD));
	CreateScore();
	CreateScore2();
	CreateButterfree();
	CreateTrail();
	...
	// Coins will be awarded at game over
	sFlappy->timer = 5;
