...
struct Snake {
	...
	u8 NextDirection; // Buffered input for next move
	u8 LastDirection;
	u32 FrameCount;
	u32 delay;
	u8 CanMove;
...
static void CreateOnix(void)
{
	struct SpriteSheet s;
        LZ77UnCompWram(sSpriteSheet_Onix.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateBody(s16 x, s16 y)
{
	...
	if (Body == 0)
	{
        LZ77UnCompWram(sSpriteSheet_Onix_Body.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
	...
	{
        LZ77UnCompWram(sSpriteSheet_Onix_Body_2.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
	...
	{
        LZ77UnCompWram(sSpriteSheet_Onix_Body_3.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
	...
	{
        LZ77UnCompWram(sSpriteSheet_Onix_Body_4.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateBerry(void)
{
	...
	if (Berry == 0)
	{
        ...
        LZ77UnCompWram(sSpriteSheet_Berry_1.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
	...
	{
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Berry_2.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
	...
	{
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Berry_3.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateCreditSprites(void)
{
    ...
    {
        struct SpriteSheet s;
        LZ77UnCompWram(sSpriteSheets_CreditsInterface[i].data, sDecompressionBuffer);
        s.data = sDecompressionBuffer;
...
static void CreateStartMenu(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Start.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateGameOver(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_GameOver.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void CreateMenu(void)
{
	struct SpriteSheet s;
		...
		LoadSpritePalettes(sSpritePalettes);
        LZ77UnCompWram(sSpriteSheet_Menu.data, sDecompressionBuffer);
		s.data = sDecompressionBuffer;
...
static void SnakeMain(u8 taskId)
{
	...
	// Buffer input every frame for responsive controls
	if (sSnake->state == SNAKE_STATE_MOVEMENT_LOOP || sSnake->state == SNAKE_STATE_PROCESS_INPUT || sSnake->state == SNAKE_STATE_SPRITE_UPDATE)
	{
		if (JOY_NEW(DPAD_UP) && sSnake->LastDirection != DOWN)
			sSnake->NextDirection = UP;
		else if (JOY_NEW(DPAD_DOWN) && sSnake->LastDirection != UP)
			sSnake->NextDirection = DOWN;
		else if (JOY_NEW(DPAD_LEFT) && sSnake->LastDirection != RIGHT)
			sSnake->NextDirection = LEFT;
		else if (JOY_NEW(DPAD_RIGHT) && sSnake->LastDirection != LEFT)
			sSnake->NextDirection = RIGHT;
	}
	...
	switch (sSnake->state)
	{
		...
		case SNAKE_STATE_PROCESS_INPUT:
			if (sSnake->delay == 0)
			{
				...
				sSnake->Direction = sSnake->NextDirection;
...
{	
	...
	sSnake->NextDirection = DOWN;
	sSnake->LastDirection = DOWN;
	sSnake->Body1Direction = DOWN;
	sSnake->Body1LastDirection = DOWN;
	sSnake->Body2Direction = DOWN;
	sSnake->Body2LastDirection = DOWN;
	sSnake->Loop = 2;
